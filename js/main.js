...
async function fetchSlots() {
  ...
  let res=null;
  if(localStorage.getItem('pincode')!==null && localStorage.getItem('pincode')!==undefined){
    res = await axios({
      method: 'get',
  ...
  }else if(localStorage.getItem('district_id')!==null && localStorage.getItem('district_id')!==undefined){
    res = await axios({
      method: 'get',
...
async function check(centers) {
  ...
  available = centers.filter(center => {
    ...
    return center['sessions'].some(s => {
      if (vaccine && vaccine.toUpperCase() !== s['vaccine'].toUpperCase())
        return false
  ...
  await delay(1)
  if (count === 0) {
    results.innerHTML = `<div class="alert alert-danger">No centers listed for ${age}+ age group found at your pincode 😔</div>`
  } else if (available.length === 0) {
    results.innerHTML = `<div class="alert alert-danger">Found ${count} centers listed for ${age}+ age group in your pincode, and all of them are fully booked right now 😕</div>`;
  } else {
    results.innerHTML = `<div class="alert alert-success">Found <b>${count} centers</b> listed for ${age}+ age group your pincode, out of which <b>${available.length} centers</b> have available slots.</div>`
...
async function setPreferences() {
  ...
  pref = {
    dose,
    ben: ben?ben.filter(b => b.allow).map(b => b['beneficiary_reference_id']):[]
  }
  showBeneficiaries()
}
